...
- External Agents：Codex Desktop、n100 上的 OpenHuman/CLI Agent 或其他可调用 HTTP 的执行器。
...
N100 当前项目目录为 `/home/aidi/apps/osint-agent-network`，备份目录为 `/home/aidi/backups/osint-agent-network`。Web 构建使用 `frontend/.env.production` 固定 `VITE_API_BASE_URL=http://10.0.0.184:8088`，避免浏览器从 3008 端口误请求 API。
...
- System status: `http://10.0.0.184:8088/api/system/status`
- Tool health: `http://10.0.0.184:8088/api/tools/health`
...
n100 部署时建议把项目放在 `/home/aidi/apps/osint-agent-network`，并把 `.env` 中的工具路径、模型中转地址和 API Token 配好。
...
- [docs/UPDATE_LOG.md](docs/UPDATE_LOG.md): 更新日记、修复摘要和 n100 部署记录
...
- [docs/N100_DEPLOYMENT_RUNBOOK.md](docs/N100_DEPLOYMENT_RUNBOOK.md): n100 部署、systemd、烟测、备份和回滚 Runbook
- [docs/REAL_TOOL_ENABLEMENT.md](docs/REAL_TOOL_ENABLEMENT.md): n100 真实 OSINT 工具接线、缺口和验收清单
...
项目已经具备部署闭环：任务创建、Agent 队列、工具适配器协议、SQLite 持久化、情报循环调度、交叉验证、图谱展示、白皮书报告、质量闸门、Token 保护、验证脚本和 n100 systemd Runbook。当前公开发布协议已经固定为 GNU GPL v3 (`GPL-3.0-only`)。下一步最值得继续增强的是更多真实工具产物样本、PDF/HTML 导出模板、权限分层和审计日志。

...
const requiredTaskTenCopy = ["尚未选择调查任务", "创建调查任务", "关闭节点详情"];
const missingTaskTenCopy = requiredTaskTenCopy.filter((phrase) => !combined.includes(phrase));
if (missingTaskTenCopy.length) {
  console.error(`任务与图谱界面缺少中文文案: ${missingTaskTenCopy.join(", ")}`);
  process.exit(1);
}
